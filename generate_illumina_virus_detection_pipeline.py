...
def main():
  ...
  parser.add_argument("-v", "--version", help="Tells you the version of script",
                      action="store_true")
  parser.add_argument("-s", "--sample_file", help="Takes in the data that you wish to use to run FastQC on",
                      type=str)
  ...
  for key in d1:
    ...
    #####
    f.write(f"{args.output_directory}/{key}/diamond_fastq1.OK: {args.output_directory}/generate_db.OK\n")
    f.write(f"\tzcat {sample_object.fastq1} | seqtk seq -A > {args.output_directory}/{key}/input{key}1.fasta\n")
    f.write(f"\tcat {args.output_directory}/{key}/input{key}1.fasta | grep " + '">"' + f" | wc -l > {args.output_directory}/{key}/count1.txt\n")
    f.write(f"\t{diamond} blastx -d {args.output_directory}/nr.dmnd -q {args.output_directory}/{key}/input{key}1.fasta -o {args.output_directory}/{key}/stitle{key}1.m8 -f 6 qseqid stitle > {args.output_directory}/{key}/diamond1.log 2> {args.output_directory}/{key}/diamond1.err\n")
    ...
    #####
    f.write(f"{args.output_directory}/{key}/diamond_fastq2.OK: {args.output_directory}/generate_db.OK\n")
    f.write(f"\tzcat {sample_object.fastq2} | seqtk seq -A > {args.output_directory}/{key}/input{key}2.fasta\n")
    f.write(f"\tcat {args.output_directory}/{key}/input{key}2.fasta | grep " + '">"' + f" | wc -l > {args.output_directory}/{key}/count2.txt\n")
    f.write(f"\t{diamond} blastx -d {args.output_directory}/nr.dmnd -q {args.output_directory}/{key}/input{key}2.fasta -o {args.output_directory}/{key}/stitle{key}2.m8 -f 6 qseqid stitle > {args.output_directory}/{key}/diamond2.log 2> {args.output_directory}/{key}/diamond2.err\n")
    ...
    f.write(f"\tcat {args.output_directory}/{key}/{key}_1.fq | grep " + '"@"' + f" | sed 's/ /\t/' | cut -f1 > {args.output_directory}/{key}/1_fq_accession.txt\n")
    f.write(f"\tcat {args.output_directory}/{key}/{key}_2.fq | grep " + '"@"' + f" | sed 's/ /\t/' | cut -f1 > {args.output_directory}/{key}/2_fq_accession.txt\n")
    ...
    f.write(f"\t{blastn} -db {refseq} -query {args.output_directory}/{key}/trinity_assembly.Trinity.fasta  -outfmt " + 
    ...
    #bowtie2 = "/usr/local/bowtie2-2.4.4/"
    #{args.output_directory}/{key}/bowtie2.OK
